...
}},{utterance:'Pack snacks for six people'});
const assistant=new Assistant({session:app.session,planner:{decide:()=>{throw Error('Offline fixture: no planner fallback');}}});assistant.functions=app.functions;
for(const [index,utterance] of process.argv.slice(2).entries()){
  const started=performance.now(),result=await assistant.execute('fixture-'+index,utterance);
  console.log(JSON.stringify({utterance,status:result.status,output:result.functionOutput||null,elapsedMs:Math.round(performance.now()-started)}));
}
